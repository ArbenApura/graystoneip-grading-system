<script lang="ts">
	// IMPORTED TYPES
	import type { CourseStudentData, CriteriaItemData } from '$types/curriculum';
	// IMPORTED UTILS
	import { generateId } from '$utils/helpers';
	import { createErrorModal } from '$stores/modalStates';
	import { insertCriteriaGrade } from '$utils/supabase';
	...
	const handleChange = (val: string) => {
		let v = parseInt(val);
		if (v < 0) v = 0;
		else if (v > criteria_item.total) v = criteria_item.total;
		score = v.toString();
	};
...
<Modal open={true} permanent={true} class="w-full" size="sm">
	<svelte:fragment slot="header">
		<div class="w-full flex items-center gap-4">
			...
			<p class="text-xl text-black flex-grow">Add Score</p>
	...
	<svelte:fragment slot="footer">
		<div class="w-full flex items-center justify-end gap-4">
			...
			<Button size="sm" color="green" disabled={isLoading} on:click={handleSave}>
				{#if isLoading}
