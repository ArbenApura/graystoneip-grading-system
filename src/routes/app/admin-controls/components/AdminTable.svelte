<script lang="ts">
	...
	// PROPS
	export let admins: Account[], openEditorModal: (account: Account) => void;
	...
	$: start = page * limit;
	$: end = (page + 1) * limit;
	$: filtered = admins.slice(start, end);
	...
	<div class="text-xs flex items-stretch flex-col sm:flex-row gap-4 py-4">
		...
		<div class="w-full flex items-center gap-4">
			<p class="flex-grow flex items-center">
				{start + 1}-{admins.length < end ? admins.length : end} of {admins.length}
				items
			</p>
			<ButtonGroup>
				...
				<Button on:click={handleNext} disabled={admins.length < end}>
