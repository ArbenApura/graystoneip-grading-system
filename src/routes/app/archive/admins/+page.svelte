<script lang="ts">
	...
	let localStorageKey = 'config.archive.admins';
	...
	$: rowItems = items.map((item) => {
		...
		const tools: RowTool[] = [
			{
				label: 'Unarchive Admin',
				icon: 'ph-bold ph-arrow-counter-clockwise',
				handleClick: () =>
					createConfirmationModal({
						message: 'Are you sure you want to unarchive this admin account?',
						handleProceed: () =>
							createVerificationModal({
	...
	const handleRefresh = async () => {
		loading = true;
		try {
			items = await selectAccounts({ type: 'admin', search, is_archived: true });
		} catch (error: any) {
