<script lang="ts">
	...
	// PROPS
	export let professors: Account[], openEditorModal: (account: Account) => void;
	...
	$: start = page * limit;
	$: end = (page + 1) * limit;
	$: filtered = professors.slice(start, end);
	...
	<div class="text-xs flex items-stretch flex-col sm:flex-row gap-4 py-4">
		...
		<div class="w-full flex items-center gap-4">
			<p class="flex-grow flex items-center">
				{start + 1}-{professors.length < end ? professors.length : end} of {professors.length}
				items
			</p>
			<ButtonGroup>
				...
				<Button on:click={handleNext} disabled={professors.length < end}>
